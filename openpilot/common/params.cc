...
#ifdef _WIN32
    // symlinks need privileges on Windows; a plain directory does for development
    if (_mkdir(key_path.c_str()) != 0 && errno != EEXIST) {
      return false;
    }
#else
...
#ifdef _WIN32
    // an open file cannot be renamed on Windows
    close(tmp_fd);
    tmp_fd = -1;
#endif
...
void Params::clearAll(ParamKeyFlag key_flag) {
  ...
  for (const auto &entry : std::filesystem::directory_iterator(getParamPath(), ec)) {
    ...
    if (it == keys.end() || (it->second.flags & key_flag)) {
      unlink(getParamPath(name).c_str());
    }
  }
#else
