...
optional overrides. The route must carry reprojectCalibration with the applied rotation.
  openpilot/tools/reproject_c4/replay.py <route> [-d data_dir] [-s start_s] [--view default|reprojected]
...
on this PC from what they logged. It follows replay's camera streams and the logged reprojectCalibration: reprojects each frame
pair into the comma 4 geometry at the applied rotation (served as "reproject").
...
from openpilot.selfdrive.modeld.reprojectd import NARROW, WIDE
from openpilot.system.camerad.cameras.nv12_info import get_nv12_info

DEV = os.environ['DEV']
C4_CAM = RC.C4_CAM
CACHE_DIR = RC.table_cache_dir()
FitStatus = custom.ReprojectCalibration.Status
VIEWS = {'default': 0, 'reprojected': 1}
...
class Stage:
  ...
  a rotation the log says is coming (reprojectCalibration building or ready) is built ahead in a thread."""
  def __init__(self, src_wh, rotation):
    ...
    self.rp = Reprojector(T, C4_CAM, DEV)
  ...
  def build_ahead(self, rotation) -> None:
    ...
    if key == self.rotation or key in self.ahead:
      return
  ...
  def follow(self, rotation) -> None:
    ...
    if rot == self.rotation:
      return
    t = self.ahead.pop(rot, None)
    if t is not None:
      t.join()
    t0 = time.perf_counter()
    self.calib = RC.calib_from_rotvec(rot)
    T = RC.load_tables(self.src_wh, C4_CAM, CACHE_DIR, self.calib)
    self.rp.reload(T)
    self.meter = RC.SeamMeter(T["meter"])
    self.rotation = rot
...
def serve():
  sm = messaging.SubMaster(['reprojectCalibration', 'narrowRoadCameraState', 'wideRoadCameraState'])
  ...
  src_size = get_nv12_info(*src_wh)[3]
  print(f"replay stage: cameras {src_wh[0]}x{src_wh[1]}; waiting for reprojectCalibration (the route must carry it)")
  while True:
    sm.update(100)
    if sm.seen['reprojectCalibration'] and len(sm['reprojectCalibration'].applied) == 3:
      break
  stage = Stage(src_wh, sm['reprojectCalibration'].applied)
  ...
  while True:
    ...
    if fit.status in (FitStatus.building, FitStatus.ready) and len(fit.target) == 3:
      stage.build_ahead(fit.target)
    if sm.updated['reprojectCalibration'] and len(fit.applied) == 3:
      stage.follow(fit.applied)
    ...
    wide_np = np.frombuffer(buf_w.data, dtype=np.uint8, count=src_size)
    narrow_np = np.frombuffer(buf_n.data, dtype=np.uint8, count=src_size)
...
def seed_params(route: str, data_dir: str | None, view: str | None, debug: str | None) -> None:
  ...
  print(f"replay: params from {os.path.basename(os.path.dirname(path))}; view {params.get('ReprojectionView', return_default=True)}, "
        + f"debug {params.get('ShowReprojectionDebug', return_default=True)}")


def run(a):
  seed_params(a.route, a.data_dir, a.view, a.debug)
  ...
  cmd = [os.path.join(BASEDIR, "openpilot/tools/replay/replay"), a.route, "--wide-road", "-s", str(a.start)]
  if a.data_dir:
    cmd += ["-d", a.data_dir]
  try:
...
def main():
  ...
  ap.add_argument("--debug", choices=DEBUG)
