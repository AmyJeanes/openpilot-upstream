...
CAMERAD = "camerad"
REPROJECT = "reproject"  # reprojectd's comma 4 frames: ReprojectionView 1, shown whole, letterboxed, never panned
DEBUG_FRAMES = {CAMERAD: "device", REPROJECT: "c4"}  # the debug view's frame names
...
class AugmentedRoadView(CameraView):
  def __init__(self, stream_type: VisionStreamType = VisionStreamType.VISION_STREAM_NARROW_ROAD):
    ...
    self._debug_frame = "device_narrow"
    self._debug_transform: np.ndarray | None = None  # what is on screen, for the debug view
  ...
  def _switch_stream_if_needed(self, sm):
    stage = sm.seen['reprojectCalibration']  # no eGPU, no stage: the setting is ignored rather than showing a stream that never comes
    name = REPROJECT if stage and ui_state.reproject_view else CAMERAD
  ...
  def _calc_frame_matrix(self, rect: rl.Rectangle) -> np.ndarray:
    ...
    if self._name == REPROJECT:  # the comma 4 frames are rendered about the device axes
    ...
    try:
      if self._name == REPROJECT:  # no vanishing-point pan: the frame is shown whole
