...
class Fit:
  ...
  def frame(self, narrow_y, wide_y, frame_id: int):
    ...
    if not self.fits:
      r = RC.fit_rotation(narrow_y, wide_y, self.calib)
    else:
      r = RC.fit_rotation(narrow_y, wide_y, RC.calib_from_rotvec(self.mean), iters=1, coarse=False)
...
class FitState:
  """The fit as reprojectd, calibrationd, the alert, the ui and the log see it (reprojectCalibration): published on every change and
