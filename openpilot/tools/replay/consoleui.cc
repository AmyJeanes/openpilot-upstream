...
#ifdef _WIN32
  // PDCurses keeps reporting the resize until resize_term() acknowledges it
  if (!is_termresized()) return;
  resize_term(0, 0);
  if (LINES == max_height && COLS == max_width) return;
#else
...
void ConsoleUI::updateStatus() {
  ...
  const char *time_string = ctime(&cur_ts);  // NULL for out-of-range times on Windows
  std::string current_segment = " - " + std::to_string((int)(replay->currentSeconds() / 60));
  write_item(0, 25, "TIME:  ", time_string ? time_string : "?", current_segment, true);
...
void ConsoleUI::updateTimeline() {
  ...
  for (const auto &entry : *replay->getTimeline()) {
    ...
    // chgat keeps the cell's character and takes the color as a pair number, not as attribute bits;
    // the markers use the same '_' as the legend
